...
class HomeViewController: BaseViewController {
    ...
    var postListType = PostListType.home
    var index:Int!
    ...
    func setupNavigationBar() {
        ...
        case .explore:
            setupNavBarWith(title: "Explore")
        case .myPost:
            setupNavBarWith(title: "My Post")
        case .savedTopic:
            setupNavBarWith(title: "Saved Topics")
        }
    }
    ...
    @objc func handleRefresh(_ refreshControl: UIRefreshControl) {
        ...
        if postListType == .savedTopic {
            viewModel.getBookmarkedPostBy(pageCount: 1, lastCreatedAt: "")
        } else {
            viewModel.getPostBy(pageCount: 1, lastCreatedAt: "")
        }       
    }
...
extension HomeViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if indexPath.row == viewModel.totalRow - 1 && !viewModel.isLastContent {
            if !viewModel.isLastContent {
                ...
                if postListType == .savedTopic {
                    viewModel.getBookmarkedPostBy(pageCount: viewModel.pageCount, lastCreatedAt: viewModel.posts[indexPath.row].id)
                } else {
    ...
    func tableView(_ tableView: UITableView, didEndDisplaying cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        self.updateCell(at: indexPath, pagerCollection: cell.feedCollectionView)
...
//            self.stopVideoPlay(at: indexPath, pagerCollection: cell.feedCollectionView)
                //mmPlayerLayer.player?.pause()
//        mmPlayerLayer.getStatusBlock { [self] (status) in
//             print(status)
...
extension HomeViewController: MZAlertViewDelegate {
...
extension HomeViewController {
    func playAudio (_ urlString : String) {
        ...
        downloadTask = URLSession.shared.downloadTask(with: url, completionHandler: { [self] url, response, error in
            do {
